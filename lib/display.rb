    ...
    if ARGV.join.include?("--network")
      @connect = Button.new(window: self, text: "Connect to Plotter", x: 100, y: @machine.bed.y+@machine.bed.height+100) do
        if @connection
          ...
          @connection = Connection.new(machine: @machine)
        end
      end
      @left_x  = Button.new(window: self, text: "←", x: 350, y: @machine.bed.y+@machine.bed.height+100, enabled: false, holdable: true, released: proc{@connection.request("move #{@machine.plotter.pen_x}:#{@machine.plotter.pen_y}"); @machine.status(:busy, "Moving to #{@machine.plotter.pen_x}:#{@machine.plotter.pen_y}"); @machine.canvas.refresh}) {@machine.pen.x = @machine.pen.x-1; @machine.pen.update}
      @right_x = Button.new(window: self, text: "→", x: 380, y: @machine.bed.y+@machine.bed.height+100, enabled: false, holdable: true, released: proc{@connection.request("move #{@machine.plotter.pen_x}:#{@machine.plotter.pen_y}"); @machine.status(:busy, "Moving to #{@machine.plotter.pen_x}:#{@machine.plotter.pen_y}"); @machine.canvas.refresh}) {@machine.pen.x = @machine.pen.x+1; @machine.pen.update}
      @up_y    = Button.new(window: self, text: "↑", x: 410, y: @machine.bed.y+@machine.bed.height+100, enabled: false, holdable: true, released: proc{@connection.request("move #{@machine.plotter.pen_x}:#{@machine.plotter.pen_y}"); @machine.status(:busy, "Moving to #{@machine.plotter.pen_x}:#{@machine.plotter.pen_y}"); @machine.canvas.refresh}) {@machine.pen.y = @machine.pen.y-1; @machine.pen.update}
